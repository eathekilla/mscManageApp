...
export class MagicianComponent implements OnInit {
  ...
  sortByLastName(){
    this.array.sort((a,b) => a.name.split(" ",2)[1].localeCompare(b.name.split(" ",2)[1]));
  }
  ...
  searchMagician(term: string): Observable<Magician[]> {
    ...
    return this.http.get<Magician[]>(`${this.array}/?name=${term}`).pipe(
      tap(x => x.length ?
  ...
  sortHouse(){
    this.searchTerms.next("Gryffindor");
    this.magician$ = this.searchTerms.pipe(debounceTime(300),distinctUntilChanged(),switchMap((term: string) => this.magicianService.searchMagician("Gryffindor")));
